...
fn test() {
    ...
    let frame: Vec<u8> = encoder
        .encode(
            Box::new(
                crate::server::prelude::versions::v1::c0x0003::ComponentNeedsResponse::new(
                    "test".to_string(),
                    None,
                    vec!["test.html".to_string(), "text.css".to_string()]
                )
            )
        )
        .unwrap();
    ...
    let mut decoded_data =
        crate::client::prelude::versions::v1::r0x0003::ComponentNeedsResponse::new(
            decoder
        );
    ...
    assert_eq!(decoded_data.component_name, "test".to_string());
    assert_eq!(decoded_data.title, None);
    assert_eq!(
        decoded_data.files,
        vec!["test.html".to_string(), "text.css".to_string()]
    );
