...
fn test() {
    ...
    let frame: Vec<u8> = encoder
        .encode(
            Box::new(
                crate::server::prelude::versions::v1::c0x0002::ErrorResponse::new(
                    Error {
                        code: 404,
    ...
    let mut decoded_data =
        crate::client::prelude::versions::v1::r0x0002::ErrorResponse::new(
            decoder
        );
    ...
    assert_eq!(decoded_data.code, 404);
    assert_eq!(decoded_data.message, "File not found".to_string());
}
